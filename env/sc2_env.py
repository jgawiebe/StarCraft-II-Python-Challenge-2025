...
class SC2Env(environment.Base):
  ...
  def __init__(self,
    ...
    Args:
      ...
      render: Whether to render the actual SC2 game window. Set ``False`` to
          run the game in headless mode without displaying the window.
  ...
  def _launch_game(self):
    ...
    extra_args = ["-headlessNoRender"] if self._headless else None
    self._sc2_procs = [
  ...
  def _observe(self, target_game_loop):
    ...
    self.outcome = [0] * self._num_agents
      ...
      for i, o in enumerate(self._obs):
        ...
        for result in o.player_result:
          if result.player_id == player_id:
            self.outcome[i] = possible_results.get(result.result, 0)
    ...
    if self._state == environment.StepType.LAST:
      ...
      logging.info(("Episode %s finished after %s game steps. "
                   ...
                   self._episode_count, self._episode_steps, self.outcome, reward,
  ...
  def save_replay(self, replay_dir, name="player1"):
    prefix = f"{self._map_name}_{name}"
